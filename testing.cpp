...
static void TestingComposite();
static void TestingCommand();
static void TestingAdapter();
...
TEST_CASE("Adapter Pattern Test") {
    std::cout << std::endl;
...
TEST_CASE("Template Pattern Test") {
    ...
    // Creating a PoliciesDepartment and testing policy implementation
    PoliciesDepartment* pol = new PoliciesDepartment(city);
    pol->implementPolicy("Smack bad people");
    pol->Report();

    // Creating a TaxationDepartment and testing tax update
    TaxationDepartment* tax = new TaxationDepartment(city);
    tax->updateTaxes(20);
    tax->Report();
